...
document.addEventListener("DOMContentLoaded", function () {
  ...
  for (i = 0; i < tab.length; i++) {
    tab_list[i] = tab[i].getElementsByClassName("tab-list")[0].getElementsByTagName("a");
    var tab_index = tab[i].getElementsByClassName("current")[0].id.split("-")[1];
    ...
    for (j = 0; j < tab_list[i].length; j++) {
      tab_list[i][j].onclick = function () {
        ...
        for (k = 0; k < tab_list.length; k++) {
          ...
          tab_content = tab_content = tab.querySelector("#content-" + tab_index);
...
function DropdownToggle() {
  ...
  for (i = 0; i < dropdown.length; i++) {
    if (event.target === dropdown[i] || event.target === dropdown[i].childNodes[1]) {
